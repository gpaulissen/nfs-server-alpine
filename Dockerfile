...
RUN apk add --no-cache --update --verbose nfs-utils bash iproute2 net-tools && \
...
RUN chmod +x /usr/bin/nfsd.sh && \
		{ test "$NFS_GID" -eq 0 || test "$NFS_UID" -eq 0 || addgroup --gid $NFS_GID $NFS_GRP } && \
		{ test "$NFS_GID" -eq 0 || test "$NFS_UID" -eq 0 || adduser --no-create-home --disabled-password --gecos "" --uid $NFS_UID --ingroup $NFS_GRP $NFS_USR } && \
		mkdir /nfs && \
